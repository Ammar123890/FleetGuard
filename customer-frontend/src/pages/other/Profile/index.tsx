...
const EditProfileForm = ({ customerDetails, onSave }) => {
	const [editedDetails, setEditedDetails] = useState({
	  profilPic: customerDetails.profilPic,
	  name: customerDetails.name,
	  companyName: customerDetails.companyName,
	  phone: customerDetails.phone,
	  location: {
		country: customerDetails.location.country,
		address: customerDetails.location.address,
		city: customerDetails.location.city,
	  },
	});
    ...
    const fetchShipments = async () => {
      try {
        ...
        if (response.status) {
          ...
          console.error('Failed to fetch shipments:', response.error);
        }
      } catch (error) {
...
const ProfilePages = () => {
	...
	const handleSaveProfile = async (updatedDetails: SetStateAction<null>) => {
		try {
		  const token = localStorage.getItem('token');
